...
const OrderList = () => {
  ...
  async function updateStatus(e, id) {
    ...
    try {
      await axios.put(
        `http://localhost:9090/api/invoices/${id}/status/${e.target.value}`
      );
      alert(
        "Đơn hàng #" + id + " " + getInvoiceStatusNotify(e.target.value) + "."
      );
    } catch (error) {
      alert(error);
    }
  ...
  return (
    <div className="order-list">
        ...
        {invoice && invoice.map((item, index) => (
          <tr>
            ...
            <td>
              <p
                onClick={() => {
                  openInvoiceDetailPopup();
                  setInvoiceDetail(item.details);
                }}
              >
                Xem
              </p>
            ...
            <td>
              ...
              >
                {invoiceStatus.map((status, index) => (
                  <MenuItem
                    disabled={
                      index < item.status || (item.status === 2 && index === 3)
                    }
                    value={index}
                  >
                    {status}
      ...
      <Dialog
        open={invoiceDetailPopup}
        TransitionComponent={Transition}
        keepMounted
        onClose={closeInvoiceDetailPopup}
        aria-describedby="alert-dialog-slide-description"
      >
        <DialogTitle>

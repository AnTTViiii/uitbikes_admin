...
import { Transition, dot3digits, getGender, requestStatus, getChargeRequestStatusName } from '../configs/functions';
...
function RequestList() {
    ...
    async function updateStatus(e, id) {
        ...
        try {
            await axios.put(`http://localhost:9090/api/requests/${id}/status/${e.target.value}`);
            alert('Đơn hàng #' + id + ' ' + getChargeRequestStatusName(e.target.value) + '.')
        } catch (error) {
            alert(error);
        }
    }
         ...
         <table className='table-1 request-list-table'>
                ...
                {request.map((item, index) => (
                    <tr>
                        ...
                        <td><p onClick={() => {openCustomerInfoPopup(); setCustomerInfo(item.customer)}}>{item.customer.name}</p></td>
                        <td>{dot3digits(item.money)} đ</td>
                        <td>{new Date(item.date).toLocaleString()}</td>
                        <td><p>{item.accountNumber}</p></td>
                        <td>
                            <Select className='select-box' defaultValue={item.status} size='small' onChange={(e) => updateStatus(e, item.id)}>
                                {requestStatus.map((status, index) =>  (
                                    <MenuItem disabled={index < item.status || (item.status === 1 && index === 1)} value={index}>{status}</MenuItem>
                                ))}
                            </Select>
                        </td>
                    </tr>
